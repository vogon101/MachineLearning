...
object DL4JTest extends App {
  ...
  if (command == "train") {
    ...
    while(true) {
      val image = readLine("Image > ")

      try {

        val imageData = Utils.readBWBMP(folder + image + ".bmp", 28)
        val out = model.output(new NDArray(imageData.map(_.toFloat)))

        var bestGuess = (-1d, -1)
        for ((x, i) <- out.dup().data().asDouble().zipWithIndex) {
          println(f"$i - $x%1.2f")
          if (x > bestGuess._1) bestGuess = (x, i)
        }

        println(f"I guess that that image was a ${bestGuess._2} with a probability of ${bestGuess._1}%1.4f")

      } catch {
        case e: IIOException => println(s"File ${folder + image}.bmp not found")
      }
    }
